...
import { ArrowLeft, Send, Paperclip, Mic, MicOff, X, Loader2, StopCircle } from "lucide-react";
...
export default function ChatWindow({ chatId }: { chatId: string }) {
  ...
  const handleTyping = () => {
    ...
    typingTimer.current = setTimeout(
      () => sendTyping(chatId, session?.user?.name || "", false),
      1500
    );
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.error || "Failed to attach media");
    } finally {
    ...
    try {
      ...
      if (data.success && data.data?.url) {
        ...
        if (res.data.success) {
          ...
          setMessages((prev) => {
            const exists = prev.some((m: any) => m._id === res.data.data._id);
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-screen bg-[#faf8f4] dark:bg-zinc-950">
  ...
  return (
    <div className="flex flex-col h-screen bg-[#faf8f4] dark:bg-zinc-950">
      {/* Header */}
      <div
        className="px-4 py-3 flex items-center gap-3 flex-shrink-0 shadow-md"
        style={{ background: "linear-gradient(135deg,#8b5e2a,#1a0a00)" }}
      >
        <button
          onClick={() => router.back()}
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-bold text-white truncate flex items-center gap-2">
            <span>{isCustomer ? chat?.sellerName : chat?.customerName}</span>
            {isAdmin && (
          ...
          <div className="text-[11px] text-emerald-400 font-sans flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Direct Chat Active
          </div>
        </div>
      </div>
      ...
      {/* Goat context chip */}
      {chat && (
        <div className="px-4 py-2 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-3 flex-shrink-0">
          {chat.goatImage && (
            <img
              src={chat.goatImage}
              alt=""
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-16 text-zinc-400 dark:text-zinc-500">
            ...
            <p className="text-sm font-sans">Start the conversation! You can send messages, photos, videos and voice notes.</p>
          </div>
        )}
        {messages.map((m, i) => {
          const mine =
          ...
          return (
            ...
            >
              ...
              >
                {/* Text Content */}
                {m.text && m.text !== "📎 Media attachment" && m.text !== "🎤 Voice message" && (
                ...
                {m.attachments && m.attachments.length > 0 && (
                  <div className="space-y-2 mt-2">
                    {m.attachments.map((att, attIdx) => {
                      ...
                      const isAudio =
                        att.includes(".webm") && m.text === "🎤 Voice message" ||
                        att.includes("audio") ||
                        att.includes("voice-message");
                      return isAudio ? (
                        <div key={attIdx} className="flex items-center gap-2">
                        ...
                        <div key={attIdx} className="rounded-xl overflow-hidden bg-black max-w-sm">
                          <video
                            src={att}
                            controls
                        ...
                        <div key={attIdx} className="rounded-xl overflow-hidden max-w-sm border border-black/10">
                          <img
                            src={att}
                            alt="Attachment"
                ...
                <div
                  className={`text-[10px] mt-1 text-right ${
                    mine ? "text-white/75" : "text-zinc-400 dark:text-zinc-500"
                  }`}
                >
                  {m.createdAt
      ...
      {attachments.length > 0 && (
        <div className="px-4 py-2 bg-zinc-100 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-3 overflow-x-auto">
          {attachments.map((att, idx) => (
            <div key={idx} className="relative w-16 h-16 rounded-xl overflow-hidden border border-zinc-300 dark:border-zinc-700 flex-shrink-0 bg-black">
              {att.includes(".mp4") || att.startsWith("data:video") ? (
                <div className="w-full h-full flex items-center justify-center text-xs text-white">🎬 Video</div>
              ) : (
      ...
      {(isRecording || audioUrl) && (
        <div className="px-4 py-3 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-3">
          {isRecording ? (
            /* Recording in progress */
            <>
              <div className="flex items-center gap-2 flex-1">
                <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse flex-shrink-0" />
                <span className="text-sm font-sans text-zinc-700 dark:text-zinc-300 font-medium">
              ...
              <button
                type="button"
                onClick={cancelVoice}
                className="w-9 h-9 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                title="Cancel"
              >
                <X size={16} />
              </button>
            ...
            <>
              ...
              <audio src={audioUrl} controls className="flex-1 h-9 rounded-full min-w-0" />
              <button
                type="button"
                onClick={cancelVoice}
      ...
      {/* ── Input Bar ── */}
      {!isRecording && !audioUrl && (
        <div className="px-4 py-3 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-2 flex-shrink-0">
          <input
            type="file"
            ref={fileInputRef}
            accept="image/*,video/*"
          ...
          >
            {uploadingMedia ? <Loader2 size={16} className="animate-spin text-[#c8a96e]" /> : <Paperclip size={16} />}
          </button>
