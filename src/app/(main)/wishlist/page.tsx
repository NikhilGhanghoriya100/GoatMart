...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(({ data }) => {
        if (data.success) setGoats(data.data);
      })
  ...
  if (!session) {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-xl font-bold font-serif mb-2">{t.loginToViewWishlist}</h2>
        <p className="text-sm text-gray-400 font-sans mb-6">{t.wishlistEmptySub}</p>
        <Link
          href="/login"
  ...
  return (
    <div className="w-full max-w-[1280px] mx-auto px-3 sm:px-5 py-6 sm:py-7 overflow-x-hidden">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="flex-1 text-center text-xl font-bold font-serif">
          {t.wishlistTitle} {!loading && `(${goats.length})`}
        </h1>
        <div className="w-9" />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <div className="text-5xl mb-4">🤍</div>
          <h2 className="text-xl font-bold font-serif mb-2">{t.wishlistEmpty}</h2>
          <p className="text-sm text-gray-400 font-sans mb-6">{t.wishlistEmptySub}</p>
          <Link
            href="/shop"
            className="inline-block px-6 py-3 rounded-full bg-gradient-to-br from-[#c8a96e] to-[#8b5e2a] text-white font-bold font-sans text-sm hover:opacity-90"
